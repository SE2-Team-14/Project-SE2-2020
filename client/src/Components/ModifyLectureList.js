...
class ModifyLectureList extends React.Component {
    ...
    constructor(props) {
        super(props);
        this.state = { lectures: [], bookings: [], showModify: false, showModifySuccess: false, showModifyError: false, showDelete: false, showDeleteSuccess: false, showDeleteError: false, showChange: false, showChangeError: false, showChangeSuccess: false, lecture: '', selectedDate: '', selectedStartTime: '', selectedEndTime: '', classroomId: 0, numberOfSeats: 0 };
    }
    ...
    handleClickModify = (lecture) => {
        this.setState({ showModify: true, lecture: lecture, selectedDate:lecture.date, selectedStartTime:lecture.startingTime, selectedEndTime:lecture.endingTime, classroomId: lecture.classroomId, numberOfSeats: lecture.numberOfSeats});
    }
     ...
     */
    handleClose = () => {
        this.setState({ showModify: false, showModifyError: false, showModifySuccess: false });
    }

    handleModify = (lecture) => {
        if (this.state.selectedDate && this.state.selectedDate != '' && this.state.selectedStartTime && this.state.selectedStartTime != '' && this.state.selectedEndTime && this.state.selectedEndTime != '') {
            lecture.date = this.state.selectedDate
            lecture.startingTime = this.state.selectedStartTime
            lecture.endingTime = this.state.selectedEndTime
            lecture.classroomId = this.state.classroomId
            lecture.numberOfSeats = this.state.numberOfSeats
            API.modifyLecture(this.state.lecture).then(() => API.getAllLecturesList().then((lectures) => this.setState({ lectures: lectures, showModifySuccess: true, showModify: false })));
        } else {
            this.setState({showModify: false, showModifyError: true})
        }
    }
    ...
    handleStartTime = (time) => {
        this.setState({ selectedStartTime: moment(time).format('HH:mm')});
    }

    handleEndTime = (time) => {
        this.setState({ selectedEndTime: moment(time).format('HH:mm')});
    }
        ...
        return (
            ...
            <Jumbotron className='d-flex justify-content-around col-12 m-0 p-3'>
                <Row className='col-12 m-0 p-0'>
                    <Col>
                        <LectureListModify lecture={this.state.lectures} findCourseName={this.findCourseName} findMaxSeats={this.findMaxSeats} handleClickModify={this.handleClickModify} />
                    </Col>
                </Row>
                <Modal controlid='Modify' show={this.state.showModify} onHide={this.handleClose} animation={false}>
                    <Modal.Header closeButton>
                        <Modal.Title>Confirm modifying</Modal.Title>
                    </Modal.Header>
                    <Modal.Body>
                        ...
                        <Form.Group>
                            ...
                            <KeyboardTimePicker
                                ...
                                id="start-time"
                                format="hh:mm"
                                value={moment(this.state.selectedStartTime, "HH:mm")}
                        ...
                        <Form.Group>
                            ...
                            <KeyboardTimePicker
                                ...
                                id="end-time"
                                format="hh:mm"
                                value={moment(this.state.selectedEndTime, "HH:mm")}
                        ...
                        <Form.Group>
                            ...
                            <Form.Control type="number" value={this.state.classroomId} onChange={this.handleClassroomId}></Form.Control>
                        </Form.Group>
                        <Form.Group>
                            <Form.Label>Number of Seats:</Form.Label>
                            <Form.Control type="number" value={this.state.numberOfSeats} onChange={this.handleNumberOfSeats}></Form.Control>
                        </Form.Group>
                        </MuiPickersUtilsProvider>
                        Do you want to confirm your modifying?
                    </Modal.Body>
                    <Modal.Footer>
                        <Button variant="primary" type="submit" onClick={() => this.handleModify(this.state.lecture)}>
                            Modify
                        </Button>
                    </Modal.Footer>
                </Modal>
                <Modal controlid='ModifyError' show={this.state.showModifyError} onHide={this.handleClose} animation={false} >
                    <Modal.Header closeButton>
                        <Modal.Title>ERROR!</Modal.Title>
                    </Modal.Header>
                    <Modal.Body>Please complete your modification information!</Modal.Body>
                    <Modal.Footer>
                        <Button variant='primary' onClick={() => this.handleClose()}>Close</Button>
                    </Modal.Footer>
                </Modal>
                <Modal controlid='ModifySuccess' show={this.state.showModifySuccess} onHide={this.handleClose} animation={false} >
                    <Modal.Header closeButton>
                        <Modal.Title>Confirm modifying</Modal.Title>
                    </Modal.Header>
                    <Modal.Body>Congrats, your lecture is modified! A mail has been sent to all students!</Modal.Body>
                    <Modal.Footer>
                        <Button variant='primary' onClick={() => this.handleClose()}>Close</Button>
                    </Modal.Footer>
                </Modal>
 ...
 */
function LectureListModify(props) {
        ...
        <ListGroup>
            ...
            </ListGroup.Item>
            {
                props.lecture.map((l) =>
                    ...
                    <LectureItemModify key={l.lectureId} lecture={l} findCourseName={props.findCourseName} findMaxSeats={props.findMaxSeats} handleClickModify={props.handleClickModify} />
                )
 ...
 */
function LectureItemModify(props) {
    let courseName = props.findCourseName(props.lecture.courseId);
    let numberOfSeats = props.findMaxSeats(props.lecture.classroomId);
    props.lecture.numberOfSeats = numberOfSeats;
    ...
    return (
        <ListGroup.Item className='border mt-1'>
            <Row className='justify-content-around'>
                ...
                <Col xs={1} className='text-center'>
                    <Button variant='danger' onClick={() => props.handleClickModify(props.lecture)}>Modify</Button>
                </Col>
            </Row>
        </ListGroup.Item>
    );
